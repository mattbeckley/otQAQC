...
-  *fsuffix*.  Set this to a string that will be appended to output
   files.  This keyword is only used if you are using the module,
   ADDCRS2Header, and don't want to overwrite the original file.  Used
   in conjunction with "overwrite" parameter. 

-  *overwrite*.  Set this to 0 or 1.  This keyword is only used if you
   are using the module, ADDCRS2Header, and don't want to overwrite the
   original file.  Used in conjunction with "fsuffix" parameter.
...
-  *shortname*.  This is the name you are going to use for the OT ingest.
   In general, your directory structure will contain this name, and you
   will use this throughout the process to identify this project.
   This is listed again because you may want to pass this string into
   the actual code.
